...
| `resolving` | 锁换道或仍允许（本版锁） | 短暂停，播对/错，再生成下一组 |
...
Canvas 世界使用单调递增的 `worldDistance` 作为统一时空基准：路面纹理、车道虚线和路边物件都从它派生。树木使用稳定的世界序号决定侧别、大小和偏移，只在地平线和镜头边缘渐入渐出，不按时间切片重新随机。门的底边是地面投影点，宽度由同一深度处的路宽推导；高度与门宽共用唯一透视比例，不得单独截断。判定后旧门继续向镜头外移动，新门才从消失点出现，避免把换题读成同一扇门突然缩小。

当且仅当需求包含真实相机转向、多层遮挡、3D 角色/骨骼或动态光照时，再开独立实验分支实现 `ThreeRenderer`。单纯换成 Three.js 不会自动解决对象闪现和尺寸跳变，这些仍由世界坐标与生命周期决定。
